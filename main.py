...
class RequestApi(object):
    ...
    def all_api_request(self):
        ...
        while True:
            ...
            time.sleep(20)
        # 5 . 获取结果
        data1=self.get_result_request(taskid=taskid)
        list_data = json.loads(data1['data'])
        (path, filename) = os.path.split(self.upload_file_path)
        with open(filename+".json", 'w') as write_f:
            json.dump(list_data, write_f, ensure_ascii=False)
        txtfile = open(filename + ".txt", 'w', encoding="utf-8")
        for item in list_data:
            print(item['onebest'], file=txtfile)

# 注意：如果出现requests模块报错："NoneType" object has no attribute 'read', 请尝试将requests模块更新到2.20.0或以上版本(本demo测试版本为2.20.0)
# 输入讯飞开放平台的appid，secret_key和待转写的文件路径
if __name__ == '__main__':
    api = RequestApi(appid="******", secret_key="*****************", upload_file_path=r"audio-file.flac")
    api.all_api_request()